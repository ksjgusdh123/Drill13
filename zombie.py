...
class Zombie:
    ...
    def __init__(self, x=None, y=None):
        ...
        self.state = 'Idle'
        self.ball_count = 0
        self.tx, self.ty = 800, 800
        self.build_behavior_tree()
        self.patrol_locations = [(43, 274), (1118, 274), (1050, 494),
                                 (575, 804), (235, 991), (575, 804), (1050, 494)]
        self.loc_no = 0
    def get_bb(self):
    ...
    def escape(self):
        ...
        if play_mode.boy.x >= self.x:
            self.move_slightly_to(-200, self.y)
        else:
            self.move_slightly_to(1400, self.y)
    ...
    def build_behavior_tree(self):
        a1 = Action('Set tartget location', self.set_target_location, 500, 150) # action node 생성
        ...
        a6 = Action('소년에게서 도망', self.escape)
        c2 = Condition('소년이 공이 더 많은가?', self.is_zombie_more_ball)
        SEQ_chase_boy = Sequence('소년을 추적', c1, c2, a4)
        SEQ_escape_to_boy = Sequence('소년에게서 도망', c1, a6)
        SEL_chase_or_escape = Selector('추적 또는 도망',SEQ_chase_boy, SEQ_escape_to_boy)
        root = SEL_chase_or_escape_or_wander = Selector('추적 또는 도망 또는 배회', SEL_chase_or_escape, SEQ_wander)
